...
const move = document.querySelectorAll('#item-background');
const choosee = document.querySelectorAll('.item-background2')
const pentagon = document.getElementById('pentagon')
const itemBack = document.getElementById('item-back')
...
move.forEach((item)=>{
    item.addEventListener("click",()=>{
        changePosition(item.className)
        itemselect=item.className.split(' ')[0]
    })
})

function changePosition(name){
    choosee.forEach((item)=>{
        item.classList.toggle('hide')
    })
    pentagon.classList.toggle('hide')
    move.forEach((item)=>{
        ...
        item.classList.add('item-background2')
        item.style.pointerEvents = "none"
    })
    result.classList.remove('hide')
    result.style.fontSize='2rem'
    result.innerHTML='<h1>'+seconds+'</h1>';
    setTimeout(function(){
        seconds--;
        result.innerHTML='<h1>'+seconds+'</h1>';
        setTimeout(function(){
...
function chooseWeb(){
    ...
    const element = ['scissors','spock','paper','rock','lizard']
    const colors = ['hsl(39, 89%, 49%)', 'hsl(189, 59%, 53%)', 'hsl(230, 89%, 62%)', 'hsl(349, 71%, 52%)', 'hsl(261, 73%, 60%)']
    const images =[
    ...
    setTimeout(()=>{
        ...
        if((element[random]=='scissors' && itemselect=='paper') || (element[random]=='scissors' && itemselect=='lizard')) winOrLose(0)
        else if((element[random]=='scissors' && itemselect=='spock') || (element[random]=='scissors' && itemselect=='rock')) winOrLose(1)
        else if((element[random]==itemselect)) winOrLose(2)

        else if((element[random]=='spock' && itemselect=='scissors') || (element[random]=='spock' && itemselect=='rock')) winOrLose(0)
        else if((element[random]=='spock' && itemselect=='paper') || (element[random]=='spock' && itemselect=='lizard')) winOrLose(1)
        else if((element[random]==itemselect)) winOrLose(2)

        else if((element[random]=='paper' && itemselect=='rock') || (element[random]=='paper' && itemselect=='spock')) winOrLose(0)
        else if((element[random]=='paper' && itemselect=='scissors') || (element[random]=='paper' && itemselect=='lizard')) winOrLose(1)
        else if((element[random]==itemselect)) winOrLose(2)

        else if((element[random]=='rock' && itemselect=='scissors') || (element[random]=='rock' && itemselect=='lizard')) winOrLose(0)
        else if((element[random]=='rock' && itemselect=='paper') || (element[random]=='rock' && itemselect=='spock')) winOrLose(1)
        else if((element[random]==itemselect)) winOrLose(2)

        else if((element[random]=='lizard' && itemselect=='spock') || (element[random]=='lizard' && itemselect=='paper')) winOrLose(0)
        else if((element[random]=='lizard' && itemselect=='scissors') || (element[random]=='lizard' && itemselect=='rock')) winOrLose(1)
        else if((element[random]==itemselect)) winOrLose(2)
    },3000)
}

function winOrLose(winorloseordraw){
    const results=['LOSE','WIN','DRAW']
    if(winorloseordraw==1){
        points=points+1;
        score.textContent=points;
    }
    const color=['#FF0000','#1776FF','#00FF1F']
    result.style='--color:'+color[winorloseordraw]
    if(winorloseordraw!=2) result.innerHTML='<h1>YOU '+results[winorloseordraw]+'</h1> <a onclick="restartGame()" >PLAY AGAIN</a>'
    else result.innerHTML='<h1>'+results[winorloseordraw]+'</h1> <a onclick="restartGame()" >PLAY AGAIN</a>'
}

function destroy(){
    choosee[0].style.background='transparent'
    itemBack.style.background="hsl(237, 49%, 15%)"
    result.classList.add('hide')
    itemBack.innerHTML='';
    result.innerHTML='';
    move.forEach((response)=>{
        response.setAttribute('id','item-background')
        response.removeAttribute('item-background2')
...
function restartGame(){
    ...
    move.forEach((response)=>{
        console.log()
        if(response.className.split(' ')[0]==itemselect){
            response.classList.toggle('move')
        }
        else {
